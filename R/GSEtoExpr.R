...
get_probe_anno <- function(GPL_id="GPL9061",GPL_data=NULL){
  ...
  if(GPL_id %in% GPLlist$GPL){
    ids <- as.character(GPLlist[GPL_id,2:3]) #选择GPL_id的id and symbol
  } else {
    stop("The gene symbol header of the ",GPL_id," is not in our GPLlist! \n Please add GPL, ID,Gene symbol to the GPLlist variable.")
  }
